...
def save_training_data(data):
    file_path = os.path.join(root_dir,'training_data.json')
    try:
...
def save_weight_data(data):
    file_path = os.path.join(root_dir, 'weight_data.json')
    try:
        with open(file_path, 'w') as file:
            file.write(json.dumps(data))
...
def create_training_chart_and_save():
    ...
    file_path = os.path.join(root_dir, 'training_data.json')
    file_path2 = os.path.join(root_dir, 'chart_training.jpg')
    file_path3 = os.path.join(root_dir, 'saved_chart_image.jpg')
    if not os.path.exists(file_path):
        # Ha nem létezik a fájl, akkor ne csináljon semmit
        return
    with open(file_path, 'r') as file:
    ...
    if os.path.exists(image_file):
        ...
        img.save(file_path3, 'JPEG')
        print('Chart image saved as saved_chart_image.jpg')
    else:
        print('Error: Image file does not exist')
def create_weight_chart_and_save():
    file_path = os.path.join(root_dir, 'weight_data.json')
    file_path2 = os.path.join(root_dir, 'chart_weight.jpg')
    file_path3 = os.path.join(root_dir, 'saved_chart_weight.jpg')
    if not os.path.exists(file_path):
        # Ha nem létezik a fájl, akkor ne csináljon semmit
        return
...
class FitTrackMateApp(toga.App):
    ...
    def handle_training_statistics(self, widget):
        ...
        self.img = toga.ImageView(id='images', image=file_path2) #program inditásakor létrehozott image megjelenítése
        # Elrendezés
    ...
    def handle_weight_statistics(self, widget):
        ...
        self.img = toga.ImageView(id='images2',image=file_path2)  # program inditásakor létrehozott image megjelenítése
        # Elrendezés
        self.box = toga.Box(children=[back_button, self.img],
                            style=Pack(direction='column', padding=20))
